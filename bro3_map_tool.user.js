...
function getMapScale() {
	var sort15 = document.evaluate('//li[@class="sort15 now"]',
		document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
	if (sort15.snapshotLength != 0) {
		return 15;
	}
	
	var sort20 = document.evaluate('//li[@class="sort20 now"]',
		document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
...
function genPopupText(data, x, y) {
	...
	//[地名]([座標])/[[距離]]
	if (village == undefined) village = "不明";
	var popupText = "";
	popupText += village;
	popupText += "(" + x + "," + y + ")"
	if (distance >= 0) {
